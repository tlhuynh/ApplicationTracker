  ...
  useEffect(() => {
    scheduleRefreshRef.current = (expiresAt: string) => {
      ...
      refreshTimerRef.current = setTimeout(() => {
        // Read the latest token from localStorage at fire time rather than using a
        // closure-captured value. If a concurrent 401-retry already rotated the token,
        // the closure value would be stale and the refresh call would return 401.
