...
- **Demo mode / API switching hook** — `useApplicationRecordsApi()` returns either the real API module or an in-memory demo module based on `isDemoMode` from `useAuth()`. Pages call the hook once (`const api = useApplicationRecordsApi()`) and use `api.getAll()`, `api.create()` etc. — no `if/else` needed in page code. Both modules share the same function signatures so TypeScript is satisfied. The demo module reads/writes `sessionStorage`; the real module calls `authFetch`. Demo mode sets `isAuthenticated = true` and `user = 'Demo'` so `ProtectedRoute` passes through without changes
- **Public fetch in demo mode** — `parseExcel()` in `applicationRecords.ts` uses plain `fetch` (not `authFetch`) with `API_BASE_URL` prefix because the `/parse` endpoint is `[AllowAnonymous]` — demo users have no JWT token so `authFetch`'s Bearer header would be empty anyway, but using plain `fetch` makes the intent explicit
- **`useRef` as synchronous flag** — unlike state, ref updates (`ref.current = true`) take effect immediately within the same synchronous event. Used in `HomePage` to block the Radix `AlertDialog` `onOpenChange` callback from closing the dialog while a delete request is in flight. State (`isDeletingPending`) drives the UI (disabled buttons, spinner); the ref is only needed for the synchronous check in `onOpenChange` that runs before the next render
- **Intercepting Radix dialog close** — Radix `AlertDialog` and `Dialog` call `onOpenChange(false)` for all close triggers (Cancel button, X button, Escape key, outside click). To selectively block closure: wrap `onOpenChange` in a function that returns early under specific conditions (e.g., `if (!isOpen && isSubmitting) return`). For `AlertDialogAction`, which closes the dialog automatically via Radix before the state update renders, use a ref to check pending state synchronously inside `onOpenChange`
- **Loading overlay on auth cards** — `Card` needs `relative` class; overlay is `absolute inset-0 flex items-center justify-center rounded-lg bg-background/80 z-10` with a `Loader2` spinner. Covers the full card including header and all interactive elements (links, buttons). Only rendered when `isSubmitting` is true
