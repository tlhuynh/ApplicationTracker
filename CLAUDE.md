...
- **Excel Export**: 7-column xlsx (CompanyName, Status, AppliedDate, PostingUrl, Notes, Description, Interviews). Interviews column is a human-readable summary: `"{n} interview(s) – last: {Type} ({Outcome})"` (outcome omitted when null). Export uses `.Include(r => r.Interviews)` via `GetAllForExportAsync`; the `Interviews` nav property on `ApplicationRecord` is only populated through this explicit include — not loaded by any other query.
- **Excel Import**: ClosedXML for parsing `.xlsx` uploads; service returns domain models, controller maps to DTOs. Validation: CompanyName required, Status must match enum name (numeric values rejected), AppliedDate required (culture-invariant parsing via `CultureInfo.InvariantCulture`), PostingUrl must be valid HTTP/HTTPS if provided, Notes (col 5) max 5000 chars, Description (col 6) optional max 20,000 chars (blank → null). Duplicate detection: CompanyName + PostingUrl (when URL provided), fallback to CompanyName + AppliedDate (database check only, not within same batch). `IExcelImportService` has two methods: `ImportAsync` (parse + validate + save, requires userId) and `ParseAsync` (parse + validate only, no DB save, used by the public `/parse` endpoint for demo mode)
...
- **Branch strategy**: `dev` is the integration target — all feature branches PR into `dev`. `main` is the clean public-facing deploy branch — merge `dev` → `main` to deploy. `main` intentionally omits AI tooling files (`CLAUDE.md`, `.claude/`, etc.); merging `dev` → `main` will never re-add them (git preserves the deletion). PRs default to targeting `main` on GitHub — manually change the target to `dev` when creating feature PRs.
